...
@app.route('/api/jobinfo', methods=['GET'])
def get_job_info():
    try:
...
@app.route('/api/jobgrowth', methods=['GET'])
def get_job_growth():
    try:
        # Connect to MySQL database
        cnx = mysql.connector.connect(user='root', password='Qwer1234!',
                                      host='localhost', database='db')
        cursor = cnx.cursor()
        cursor.execute('SELECT * FROM jobgrowth')
        result = cursor.fetchall()
...
def process_skills():
    ...
    # Process the file content string
    res = upskill(file_content_string)
...
def recommend_course():
    ...
    try:
        headers = {
            'Authorization':'Basic eTJxcTNJQk1uR3VLcm16eTZRQ3VMeEFNb1RVYXV5djF5QjBPYkhpaTpCYnI3bEV3UlR6WW9JZTBPS2c3Y1NCeDJNMlREd2pjR1JUbUdJWUlrTE13aEx4dGxsbU9RSk5MNzlyZDhGWEJmYUUwQW9ub1M2MGtLRWdqSW9FNGFja0pkV29vNVYwT3VlbVZXYVJDSWxEQk1PY0JPWTdMRlVpUXRBZmVYd1RCTw=='
        }
        response = requests.get(
            'https://www.udemy.com/api-2.0/courses/?page=1&page_size=5&search=' + skill,
            headers=headers
        )
